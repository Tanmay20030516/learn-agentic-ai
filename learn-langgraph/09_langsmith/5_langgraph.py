...
# ---------- Setup ----------
load_dotenv()
os.environ["LANGSMITH_PROJECT"] = "langsmith_w_langgraph"
...
def evaluate_language(state: UPSCState):
    prompt = (
        "Evaluate the language quality of the following essay. Provide detailed feedback about grammar, "
        "vocabulary, sentence structure, and clarity. Then return ONLY a numeric score from 0-10.\n\n" + state["essay"]
    )
...
def evaluate_analysis(state: UPSCState):
    prompt = (
        "Evaluate the depth of analysis in the following essay. Provide detailed feedback about critical thinking, "
        "evidence, examples, and argument quality. Then return ONLY a numeric score from 0-10.\n\n" + state["essay"]
    )
    out = structured_model.invoke(prompt)
...
def evaluate_thought(state: UPSCState):
    prompt = (
        "Evaluate the clarity of thought in the following essay. Provide detailed feedback about organization, "
        "logical flow, coherence, and how well the arguments are presented. Then return ONLY a numeric score from 0-10.\n\n" + state["essay"]
    )
    out = structured_model.invoke(prompt)
