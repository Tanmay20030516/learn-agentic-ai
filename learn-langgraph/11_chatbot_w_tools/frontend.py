...
from backend import (
    ...
    HumanMessage, BaseMessage, AIMessage, SystemMessage, ToolMessage,
...
def convert_messages_format(messages: list[BaseMessage]) -> list[dict]:
    ...
    for message in messages:
        ...
        if len(message.content): # this is needed when during a tool call, the LLM content is empty
            new_format.append({"role": role, "content": message.content})
    return new_format

def stream_with_delay(stream: Iterator[dict[Any, Any] | Any], status_holder: dict, delay: float=0.05) -> Generator:
    ...
    for message_chunk, metadata in stream:
        ...
        if message_chunk.content and isinstance(message_chunk, AIMessage): # we stream only AI messages!
...
for chat in st.session_state["chat_history"]:
    # st.write(chat["role"])
    if chat["role"] in ("system", "tool"): continue # we do not wish to print the system prompt
...
if user_query:
    ...
    CONFIG = {
        ...
        "metadata": {
            ...
            "type": "AI chatbot (with tool access) workflow",
        },
    ...
    if (st.session_state['thread_id'] not in st.session_state["chat_titles"]) and len(st.session_state["chat_history"]) > 0:
        chat_title = title_generator.invoke(
            ...
            config={
                "metadata": {"thread_id":st.session_state["thread_id"], "type": "title generation workflow", },
